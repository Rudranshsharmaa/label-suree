...
/**
 * Parses raw extracted text to identify and normalize key food packaging declarations.
...
export function parsePackagingText(imageResults = [], qrData = null) {
  const combinedText = imageResults.map(r => r.text || '').join('\n');
  const normalizedText = combinedText.replace(/\r\n/g, '\n');

  // 1. MRP Detection
  // Matches ₹ 50.00, Rs. 120, MRP: Rs 45.00 (incl. of all taxes), etc.
  ...
  // Matches: MFD / Mfg. Date / Manufactured On / PKD / Packed On
  ...
  // Matches: EXP / Expiry Date / Best Before / Use By
  ...
  // Matches 14 consecutive digits usually preceded by FSSAI or Lic / Lic No / License
  ...
  const nutritionalData = {
    hasNutritionPanel: !!(energyMatch || proteinMatch || carbMatch || fatMatch || sugarMatch),
